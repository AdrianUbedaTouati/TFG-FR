...
def detect_classes_and_labels(df: pd.DataFrame,
    ...
    """
    Top-k por frecuencia: si force_topk no es None conservamos solo las k clases más frecuentes
    (ordenadas por % desc) y mapeamos el resto a -1 para poder filtrarlas después.
    """
    # One-hot
    ...
    if oh_cols:
        ...
        if force_topk is not None:
            ...
            print(f"[TOP-K] (one-hot) {class_names} | cobertura={cov:.2%}")
    ...
    if label_col_raw and label_col_raw in df.columns:
        ...
        if force_topk is not None:
            cov = float((label_ids >= 0).mean())
            print(f"[TOP-K] (raw) {class_names} | cobertura={cov:.2%}")
        with open(class_index_path, "w", encoding="utf-8") as f:
            json.dump({i:n for i,n in enumerate(class_names)}, f, ensure_ascii=False, indent=2)
        return label_ids, class_names
...
def train_lstm_line_cls(cfg) -> Dict:
    ...
    # Si force_topk está activo, filtra fuera del top-k
    if force_topk := getattr(cfg, "FORCE_TOPK", None):
        mask = y_all >= 0
        kept = int(mask.sum()); total = int(len(y_all))
        if kept < total:
            df = df.loc[mask].reset_index(drop=True)
            y_all = y_all[mask]
            print(f"[TOP-K] Mantengo {kept}/{total} filas ({kept/total:.2%}) tras filtrar fuera del top-{force_topk}.")
    ...
    model = LSTMLineClassifier(
        ...
        head_hidden=cfg.LSTM_HEAD_HIDDEN
    ).to(device)

...
# Helper: comprobar si Triton está disponible (necesario para torch.compile GPU)
def _triton_available(min_major=3, min_minor=0):
    try:
        import triton
        v = getattr(triton, "__version__", "0.0.0")
        parts = v.split(".")
...
def set_seed(seed: int = 1337):
    ...
    # Rendimiento razonable en GPU; en CPU evita determinism para no penalizar
...
def train_one_epoch(model, loader, device, optimizer, criterion, grad_clip: Optional[float] = None,
...
def run_training(cfg) -> Dict[str, Any]:
    ...
    # Ajustes GPU (TF32/benchmark)
    use_amp = bool(getattr(cfg, "USE_AMP", True) and use_cuda)
    if use_cuda:
        try:
    ...
    # DataLoaders (parámetros dependientes del dispositivo si no están en cfg)
    num_workers = getattr(cfg, "NUM_WORKERS", (max(2, os.cpu_count()//2) if use_cuda else 0))
    pin_memory = getattr(cfg, "PIN_MEMORY", use_cuda)
    prefetch_factor = getattr(cfg, "PREFETCH_FACTOR", (4 if use_cuda else None))
    persistent_workers = getattr(cfg, "PERSISTENT_WORKERS", bool(use_cuda and num_workers > 0))

    train_loader = DataLoader(train_ds, batch_size=cfg.BATCH_SIZE, shuffle=True,  drop_last=False,
                               num_workers=num_workers, pin_memory=pin_memory,
                               prefetch_factor=prefetch_factor if num_workers > 0 and prefetch_factor else None,
    ...
    print(f"[INFO] device={device.type} | gpu={torch.cuda.get_device_name(0) if use_cuda else '-'} | amp={use_amp} | tf32={getattr(torch.backends.cuda, 'matmul', None) and torch.backends.cuda.matmul.allow_tf32 if use_cuda else '-'} | compile={getattr(cfg,'TORCH_COMPILE', True) and use_cuda and _triton_available()}")
    print(f"[INFO] dataloader: workers={num_workers} pin_memory={pin_memory} prefetch={prefetch_factor} persistent={persistent_workers}")
   ...
   # --- Modelo
    model = NBeats(
        input_size=input_size, forecast_size=cfg.L,
        width=used_hidden, depth_per_block=used_depth, num_blocks=used_blocks
    ).to(device)

    # torch.compile sólo si hay CUDA + Triton OK
    use_compile = bool(getattr(cfg, 'TORCH_COMPILE', False)) and use_cuda
    if use_compile:
        try:
            # evita errores por Triton ausente en Windows
            import triton  # noqa
            model = torch.compile(model, mode='reduce-overhead')
        except Exception as _e:
            print(f"[WARN] torch.compile desactivado: {_e}")
            use_compile = False
    ...
    # GradScaler (nueva API) para AMP en CUDA
    scaler = torch.amp.GradScaler('cuda', enabled=use_amp)

    # (opcional) Warmup rápido para “tocar” el grafo, sin capturar entrenamiento aquí
    if use_compile:
        try:
            _example = torch.randn(2, input_size, dtype=torch.float32, device=device)
            _ = model(_example)
        except Exception as _ce:
            print(f"[WARN] Warmup falló, sigo en eager: {_ce}")
            try:
    ...
    for epoch in range(1, cfg.EPOCHS + 1):
        train_loss = train_one_epoch(
            model, train_loader, device, optimizer, criterion, cfg.GRAD_CLIP,
        ...
        print(f"Epoch {epoch:03d} | train_loss={train_loss:.6f} | "
            f"val_loss={val_loss:.6f} | val_MAE={val_mae:.4f} | val_RMSE={val_rmse:.4f}")
        if val_loss < best_val:
            best_val = val_loss; no_improve = 0
            torch.save({
                'model_state_dict': model.state_dict(),
                'config': {'H': cfg.H, 'L': cfg.L, 'width': used_hidden,
                ...
                'best_val': best_val, 'best_hp': best_hp
            }, best_path)
        else:
            no_improve += 1
    ...
    # Evaluación final
    test_loss, test_mae, test_rmse = evaluate(model, test_loader, device, criterion, use_amp=use_amp)
    ...
    # Denormalizaciones para plots/CSVs (si procede)
    ...
    # Importancia de variables en validación (perm. importance)
    imp_df = None

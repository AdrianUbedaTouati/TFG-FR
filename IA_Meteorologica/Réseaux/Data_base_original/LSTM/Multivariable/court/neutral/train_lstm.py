...
__all__ = ['train_lstm', 'run_training']
...
# AMP compatibility (prefers new torch.amp, falls back to torch.cuda.amp)
def _make_scaler_and_autocast(device_type: str, enabled: bool):
    try:
        # New API (PyTorch >= 2.0)
        Scaler = torch.amp.GradScaler
        autocast_fn = torch.amp.autocast
        scaler = Scaler(device_type, enabled=enabled)
        def autocast_ctx():
            return autocast_fn(device_type, enabled=enabled)
        return scaler, autocast_ctx
    except Exception:
        # Legacy API
        Scaler = torch.cuda.amp.GradScaler
        autocast_fn = torch.cuda.amp.autocast
        scaler = Scaler(enabled=enabled)
        def autocast_ctx():
...
def run_training(cfg):
    ...
    scaler, _autocast = _make_scaler_and_autocast('cuda', enabled=use_amp and device.type=='cuda')
...
def train_lstm(cfg):
    """Back-compat wrapper: same as run_training(cfg)."""

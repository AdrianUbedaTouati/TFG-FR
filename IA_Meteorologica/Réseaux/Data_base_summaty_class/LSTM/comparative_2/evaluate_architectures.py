...
class EvalConfig:
    ...
    ROUTE_BY_GENERAL: Dict[int, str] = field(default_factory=lambda: {1: "A", 0: "B"})
...
class ModelWrapper:
    ...
    def _load(self):
        if self.is_torchscript:
            ...
            # Intento: recuperar metadatos desde archivos vecinos
            try:
                # 1) meta.json vecino
                meta_path = self.path.replace('.tsv', '.meta.json').replace('.ts', '.meta.json')
                ...
                # 2) class_index.json vecino (formato {'0':'Label0','1':'Label1',...})
                if self.class_names is None:
                    basedir = os.path.dirname(self.path)
                    candidates = [
                        os.path.join(basedir, 'class_index.json'),
                        os.path.join(basedir, 'outputs', 'lstm_summary_line_cls', 'class_index.json'),
                    ]
                    for c in candidates:
                        if os.path.exists(c):
                            with open(c, 'r', encoding='utf-8') as f:
                                ci = json.load(f)
                            try:
                                idxs = sorted((int(k), v) for k, v in ci.items())
                                self.class_names = [v for _, v in idxs]
...
def build_dynamic_mappings(dataset_class_names: List[str], gen_names: List[str], specA_names: List[str], specB_names: List[str]):
    ds_norm = [_norm_name(x) for x in dataset_class_names]
    name_to_idx = {ds_norm[i]: i for i in range(len(ds_norm))}

    final_class_map = {}
    for sid, names in (("A", specA_names), ("B", specB_names)):
        for j, n in enumerate(names):
            nn = _norm_name(n)
            idx = name_to_idx.get(nn, None)
            if idx is None:
                for k, dn in enumerate(ds_norm):
                    if nn in dn or dn in nn:
                        idx = k; break
            if idx is None:
                idx = 0
            final_class_map[(sid, j)] = idx

    route_by_general = {}
    specA_norm = [_norm_name(x) for x in specA_names]
    specB_norm = [_norm_name(x) for x in specB_names]
    for i, gn in enumerate(gen_names):
        gnn = _norm_name(gn)
        scoreA = sum(1 for s in specA_norm if s in gnn or any(tok in gnn for tok in s.split()))
        scoreB = sum(1 for s in specB_norm if s in gnn or any(tok in gnn for tok in s.split()))
        route_by_general[i] = "A" if scoreA >= scoreB else "B"

    return route_by_general, final_class_map
...
def evaluate_three_models(df: pd.DataFrame, idx_te: np.ndarray, y_all: np.ndarray, class_names: List[str], cfg: EvalConfig, device: torch.device):
    ...
    Xg_all, mask_g = build_feature_matrix(df, cfg.FEATURE_COLS)
    ...
    gen_names = mdl_gen.class_names or ["Cloudy_Sunny", "Rainy_Snowy"]
    specA_names = mdl_A.class_names or ["Cloudy", "Sunny"]
    specB_names = mdl_B.class_names or ["Rainy", "Snowy"]

    route_by_general_dyn, final_class_map_dyn = build_dynamic_mappings(class_names, gen_names, specA_names, specB_names)

    route_map = dict(cfg.ROUTE_BY_GENERAL); route_map.update(route_by_general_dyn)
    final_map = dict(cfg.FINAL_CLASS_MAP); final_map.update(final_class_map_dyn)
    ...
    Xa_te = align_features_to_model(Xa_all[te_mask_g], mdl_A.in_features)
    Xb_te = align_features_to_model(Xb_all[te_mask_g], mdl_B.in_features)

    final_pred = np.zeros_like(y_te_local)

    inv_final = {}
    for (spec_id, spec_idx), final_idx in final_map.items():
        inv_final[final_idx] = (spec_id, spec_idx)

    inv_route = {}
    for k, v in route_map.items():
        inv_route[v] = k
    y_true_general = np.zeros_like(y_te_local)
    for i in range(len(y_te_local)):
        final_cls = int(y_te_local[i])
        spec_id, _ = inv_final.get(final_cls, ("A", 0))
        y_true_general[i] = int(inv_route.get(spec_id, 0))

    y_true_A, y_pred_A = [], []
    y_true_B, y_pred_B = [], []

    for i in range(len(Xg_te)):
        route = route_map.get(int(gen_pred[i]), "A")
        final_cls = int(y_te_local[i])
        spec_id_real, spec_idx_real = inv_final.get(final_cls, ("A", 0))

        if route == "B":
            pred_b = mdl_B.predict_classes(Xb_te[i:i+1])[0]
            final_pred[i] = final_map.get(("B", int(pred_b)), 0)
            y_true_B.append(int(spec_idx_real if spec_id_real == "B" else 0))
            y_pred_B.append(int(pred_b))
        else:
            pred_a = mdl_A.predict_classes(Xa_te[i:i+1])[0]
            final_pred[i] = final_map.get(("A", int(pred_a)), 0)
            y_true_A.append(int(spec_idx_real if spec_id_real == "A" else 0))
            y_pred_A.append(int(pred_a))

    cm = confusion_matrix(y_te_local, final_pred, len(class_names))
    rep = per_class_metrics(cm)
    rep["accuracy"] = float((y_te_local == final_pred).mean()) if len(y_te_local) else 0.0

    class_names_general = gen_names
    cm_general = confusion_matrix(y_true_general, gen_pred, num_classes=len(class_names_general))
    rep_general = per_class_metrics(cm_general)

    if len(y_true_A) > 0:
        y_true_A_arr = np.array(y_true_A, dtype=np.int64); y_pred_A_arr = np.array(y_pred_A, dtype=np.int64)
        cm_specA = confusion_matrix(y_true_A_arr, y_pred_A_arr, num_classes=len(specA_names))
    else:
        y_true_A_arr = np.array([], dtype=np.int64); y_pred_A_arr = np.array([], dtype=np.int64)
        cm_specA = np.zeros((len(specA_names), len(specA_names)), dtype=np.int64)
    rep_specA = per_class_metrics(cm_specA)

    if len(y_true_B) > 0:
        y_true_B_arr = np.array(y_true_B, dtype=np.int64); y_pred_B_arr = np.array(y_pred_B, dtype=np.int64)
        cm_specB = confusion_matrix(y_true_B_arr, y_pred_B_arr, num_classes=len(specB_names))
    else:
        y_true_B_arr = np.array([], dtype=np.int64); y_pred_B_arr = np.array([], dtype=np.int64)
        cm_specB = np.zeros((len(specB_names), len(specB_names)), dtype=np.int64)
    rep_specB = per_class_metrics(cm_specB)

    print("[ROUTING]", route_map)
    print("[FINAL_MAP]", final_map)
    print("[GEN_NAMES]", class_names_general)
    print("[SPEC_A_NAMES]", specA_names)
    print("[SPEC_B_NAMES]", specB_names)
    ...
    return {
        ...
        "general": {
            "y_true": y_true_general,
            "y_pred": gen_pred,
            "cm": cm_general,
            "report": rep_general,
            "class_names": class_names_general,
        },
        "spec_A": {
            "y_true": y_true_A_arr,
            "y_pred": y_pred_A_arr,

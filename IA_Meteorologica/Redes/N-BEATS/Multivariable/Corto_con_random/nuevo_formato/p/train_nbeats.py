...
def run_training(cfg) -> Dict[str, Any]:
    ...
    # torch.compile sólo si hay CUDA (evita error de compilador en Windows/CPU)
    if getattr(cfg, 'TORCH_COMPILE', True) and use_cuda and _triton_available():
        try:
    ...
    # Warmup para forzar compilación y capturar errores en tiempo de compilación
    try:
        if getattr(cfg, 'TORCH_COMPILE', True) and use_cuda and _triton_available():
            _example = torch.randn(2, input_size, dtype=torch.float32, device=device)
            _ = model(_example)
    except Exception as _ce:
        print(f"[WARN] torch.compile falló en warmup: {_ce}; volviendo a modo eager.")
        # Desactivar dynamo e inductor para el resto de la ejecución
        try:
